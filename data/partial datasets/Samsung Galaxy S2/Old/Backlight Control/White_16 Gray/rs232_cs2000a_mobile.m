...
function rs232_cs2000a ()
    ...
    measize = 17;
    Grayscale_max=256;
    Pattern_LUT=[0 16 32 48 64 80 96 112 128 144 160 176 192 208 224 240 255]

    specarray = zeros(measize,401);
    

    try
        S = serial('COM3','RequestToSend','off','BaudRate',9600,'DataBits',8,'StopBits',1,'FlowControl','none')
        fopen(S);
        ...
        for i=1:1:measize
            fn = my_filename(Pattern_LUT(i));
            fnmat = my_findfile(fn);
            if isempty(fnmat)
                disp('error finding file')
                return
            end
            boxSendImg(fnmat, '192.168.44.134',8080,3);
            mea(i,1)=((Grayscale_max/(measize-1))*(i-1));
            mea(i,2)=((Grayscale_max/(measize-1))*(i-1));
            mea(i,3)=((Grayscale_max/(measize-1))*(i-1));
            ...
            save([displayname '_' timestamp '.mat'],'mea','specarray');
